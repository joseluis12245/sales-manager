...
const initialState: AllState = {
 ...
 export const userManagementApi = createApi({
     ...
     baseQuery: fetchBaseQuery({baseUrl: 'http://localhost:3010/api/v1/user-management'}),
     endpoints: (builder) => ({
...
export const uploadUserInformation = createAsyncThunk(
    ...
    async (_, {getState}) => {
      ...
      const response = await request.postData('http://localhost:3010/api/v1/user-management/create_user', {'user':user.userManagement.user , 'password':user.userManagement.password})
      return response
    });
...
export const checkUserInformation = createAsyncThunk(
    ...
    async (userInformation: userManagement, thunkAPI) => {
        ...
        const request = requests
        const userData = {user: userInformation.user, password: userInformation.password};
        const response = await request.postData('http://localhost:3010/api/v1/user-management/validate_user', {'user':userData.user , 'password':userData.password})
...
export const insertUserToDB = createAsyncThunk(
    ...
    async (_, {getState}) => {
        const { user } = getState() as {user: AllState};
        const request = requests
        const response = await request.postData('http://localhost:3010/api/v1/user-management/insert_clients', {
            'id':user.userList[user.userList.length].id, 
            'firstName':user.userManagement.password})
        return response
});

const calculateRemainingDays = (currentDate: Date, dateRegistered: string | null) => {
...
const usermanagementSlice = createSlice({
    ...
    reducers:{
        ...
        addUserToList(state, action: PayloadAction<userList>){
            ...
            remainingDays: calculateRemainingDays(actualDate, action.payload.admissionDay)
        })
        }
    },
...
export const { setInformationUser, setView, setInitialState, addUserToList } = usermanagementSlice.actions;
